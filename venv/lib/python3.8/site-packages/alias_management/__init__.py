...
class AliasManagement:
  ...
  def _delete_file(self, filename: str) -> bool:
    ...
    """
    try:
      self.s3_client.Object(self.bucket_name, '%s.txt'%(filename)).delete()
      return True
    except:
      return False
  ...
  def get_aliases(self, masterUUID: str):
    """Get aliases from masterUUID
    ...
    Returns:
        array: Aliases array
    """
    return self._get_file(masterUUID).split("\n")

  def unalias(self, masterUUID: str, aliasUUID: str) -> bool:
    """Unalias
    ...
    if(file_content):
      ...
      if(aliasUUID in alias_array):
        alias_array = [alias for alias in alias_array if alias != aliasUUID]
        file_content = "\n".join(alias_array)
